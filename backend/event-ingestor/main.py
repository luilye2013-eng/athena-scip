...
RSS_FEEDS = [
    ...
    # "https://feeds.skynews.com/feeds/rss/world.xml",  # Temporarily disabled
...
def fetch_weather_alerts():
    """Fetch weather alerts from multiple global sources"""
    alerts = []
    
    # Keywords to filter out (test messages, non-weather)
    FILTER_KEYWORDS = [
        'test', 'test message', 'administrative', 'routine', 
        'practice', 'drill', 'exercise', 'statement'
    ]
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for alert in data.get('features', [])[:20]:
                ...
                # Only include meaningful weather events
                meaningful_types = [
            ...
            logger.info(f"Fetched {len(alerts)} NOAA weather alerts")
    except Exception as e:
        logger.error(f"NOAA API error: {e}")
    
    # 2. Global weather via Open-Meteo
    try:
        global_locations = [
...
def fetch_shipping_disruptions():
    """Fetch shipping disruptions from static intelligence data"""
    disruptions = [
...
def run_ingestion():
    ...
    try:
        ...
        logger.warning(f"Clear old data warning: {e}")
    ...
    for event in news_events:
        ...
        if store_item("events", event_data):
            total_stored += 1
